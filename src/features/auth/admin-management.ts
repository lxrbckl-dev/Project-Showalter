...
export type AdminView = {
  id: number;
  /** May be null for a pre-0026 legacy admin row. UI shows "(no email)". */
  email: string | null;
  name: string | null;
  active: boolean;
...
type AdminCtx = { adminId: number; email: string | null };

/**
 * Same admin resolution as `devices.ts` — email path first, credential
 * fallback for the pre-0026 NULL-email legacy row.
 */
async function requireAdmin(): Promise<AdminCtx | null> {
  ...
  if (byEmail && byEmail.active && byEmail.enrolledAt) {
    return { adminId: byEmail.id, email: byEmail.email };
  }
  ...
  if (session.credentialId) {
    ...
    if (credRow) {
      ...
      if (adminRow && adminRow.active && adminRow.enrolledAt) {
        return { adminId: adminRow.id, email: adminRow.email };
      }
    }
  }
