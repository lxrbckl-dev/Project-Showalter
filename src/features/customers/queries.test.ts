...
function seedAddress(customerId: number, address: string, lastUsedAt = NOW): number {
  const rows = db
...
function seedBooking(customerId: number, serviceId: number, status = 'completed'): number {
  const rows = db
    .insert(bookings)
    .values({
...
describe('searchCustomers', () => {
  ...
  it('empty query returns all customers ordered by last_booking_at DESC', () => {
    const cId1 = seedCustomer({ name: 'Alice', phone: '+19130000001', lastBookingAt: '2026-01-01T00:00:00Z' });
    const cId2 = seedCustomer({ name: 'Bob', phone: '+19130000002', lastBookingAt: '2026-03-01T00:00:00Z' });
  ...
  it('matches by name (case-insensitive)', () => {
    seedCustomer({ name: 'Alice Smith', phone: '+19130000001' });
    seedCustomer({ name: 'Bob Jones', phone: '+19130000002' });
  ...
  it('matches by phone', () => {
    seedCustomer({ name: 'Alice', phone: '+19133097340' });
    seedCustomer({ name: 'Bob', phone: '+19130000002' });
  ...
  it('matches by email', () => {
    seedCustomer({ name: 'Alice', phone: '+19130000001', email: 'alice@example.com' });
    seedCustomer({ name: 'Bob', phone: '+19130000002', email: 'bob@example.com' });
  ...
  it('matches by address', () => {
    const cId1 = seedCustomer({ name: 'Alice', phone: '+19130000001' });
    const cId2 = seedCustomer({ name: 'Bob', phone: '+19130000002' });
    seedAddress(cId1, '123 Oak St');
    seedAddress(cId2, '456 Maple Ave');
  ...
  it('address search is case-insensitive', () => {
    const cId1 = seedCustomer({ name: 'Alice', phone: '+19130000001' });
    seedAddress(cId1, '123 Oak Street');
  ...
  it('returns empty array when no match', () => {
    seedCustomer({ name: 'Alice', phone: '+19130000001' });
    const results = searchCustomers(db, 'zzznomatch', 25, 0);
    expect(results).toHaveLength(0);
  });

  it('includes totalBookings count', () => {
    const cId = seedCustomer({ name: 'Alice', phone: '+19130000001' });
    const svcId = seedService();
    // Need an address for FK
    seedAddress(cId, '123 Main St');
    seedBooking(cId, svcId);
    seedBooking(cId, svcId);
  ...
  it('pagination: limit and offset work correctly', () => {
    for (let i = 0; i < 5; i++) {
      seedCustomer({ name: `Customer ${i}`, phone: `+1913000000${i}` });
    }
...
describe('getCustomerFullDetail', () => {
  ...
  it('returns customer + addresses + bookings + reviews + photos', () => {
    const cId = seedCustomer({ name: 'Alice', phone: '+19130000001' });
    const addrId = seedAddress(cId, '123 Oak St');
    const svcId = seedService();
    // Use correct addressId
    ...
    const rId = seedReview(cId, { status: 'submitted', rating: 5 });
  ...
  it('addresses are sorted by last_used_at DESC', () => {
    const cId = seedCustomer({ name: 'Alice', phone: '+19130000001' });
    seedAddress(cId, 'Old Address', '2026-01-01T00:00:00Z');
    seedAddress(cId, 'New Address', '2026-04-01T00:00:00Z');
  ...
  it('bookings are sorted by start_at DESC (most recent first)', () => {
    const cId = seedCustomer({ name: 'Alice', phone: '+19130000001' });
    const svcId = seedService();
    const addrId = seedAddress(cId, '123 Main');
